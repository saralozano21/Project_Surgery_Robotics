...
void moveServos() {
  ...
  if (!yaw_initialized) {
    // Take the first yaw reading as the initial reference
    initial_yaw = Gri_yaw;
    // Align servo in the same direction as gripper
    servo_yaw.write(INITIAL_POSITION);
    yaw_initialized = true;
    Serial.print("Initial yaw set and aligned at: ");
    Serial.println(initial_yaw);
  }

  // Compute relative yaw change
  delta_yaw = Gri_yaw - initial_yaw;
  yaw = INITIAL_POSITION + delta_yaw;
